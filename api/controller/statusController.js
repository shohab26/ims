...
const findByKeyword = async (req, res) => {
    ...
    try {
        const [data, count] = await Promise.all([
            pool.query('SELECT * FROM status WHERE is_deleted=FALSE AND status ILIKE $1 ORDER BY id DESC LIMIT $2 OFFSET $3', [`%${value}%`, limit, offset]),
            pool.query('SELECT COUNT(*) FROM status WHERE is_deleted=FALSE AND status ILIKE $1', [`%${value}%`])
        ]);
        res.status(200).json(paginate(data.rows, parseInt(count.rows[0].count), page, limit));
    } catch (err) { res.status(500).json(err); }
};
...
const findDeleted = async (req, res) => {
    ...
    try {
        const [data, count] = await Promise.all([
            pool.query('SELECT * FROM status WHERE is_deleted=TRUE ORDER BY deleted_at DESC LIMIT $1 OFFSET $2', [limit, offset]),
            pool.query('SELECT COUNT(*) FROM status WHERE is_deleted=TRUE')
        ]);
        res.status(200).json(paginate(data.rows, parseInt(count.rows[0].count), page, limit));
    } catch (err) { res.status(500).json(err); }
};
...
const findById = async (req, res) => {
    try {
        const result = await pool.query('SELECT * FROM status WHERE id=$1', [req.params.id]);
...
const deleteById = async (req, res) => {
    try {
        const rowCount = await softDeleteById(pool, 'status', req.params.id, req.user.id);
        if (rowCount === 0) return res.status(404).json({ message: 'status id does not match.' });
...
const restoreByIdHandler = async (req, res) => {
    try {
        const rowCount = await restoreById(pool, 'status', req.params.id);
        if (rowCount === 0) return res.status(404).json({ message: 'status id does not match or is not deleted.' });
        res.status(200).json({ message: 'status restored sucessfully.' });
    } catch (err) { res.status(500).json(err); }
};
