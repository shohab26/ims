...
const findAll = async (req, res) => {
    ...
    try {
        const [data, count] = await Promise.all([
            pool.query('SELECT * FROM status ORDER BY id DESC LIMIT $1 OFFSET $2', [limit, offset]),
            pool.query('SELECT COUNT(*) FROM status')
        ]);
...
const findByKeyword = async (req, res) => {
    ...
    try {
        const [data, count] = await Promise.all([
            pool.query('SELECT * FROM status WHERE status ILIKE $1 ORDER BY id DESC LIMIT $2 OFFSET $3', [`%${value}%`, limit, offset]),
            pool.query('SELECT COUNT(*) FROM status WHERE status ILIKE $1', [`%${value}%`])
        ]);
        res.status(200).json(paginate(data.rows, parseInt(count.rows[0].count), page, limit));
    } catch (err) { res.status(500).json(err); }
};
...
const deleteById = async (req, res) => {
    try {
        const result = await pool.query('DELETE FROM status WHERE id=$1', [req.params.id]);
        if (result.rowCount === 0) return res.status(404).json({ message: 'status id does not match.' });
