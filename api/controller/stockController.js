...
const findAll = async (req, res) => {
    ...
    try {
        const [data, count] = await Promise.all([
            pool.query('SELECT * FROM stocks ORDER BY id DESC LIMIT $1 OFFSET $2', [limit, offset]),
...
const findByKeyword = async (req, res) => {
    ...
    try {
        const [data, count] = await Promise.all([
            pool.query('SELECT * FROM stocks WHERE CAST(quantity AS TEXT) ILIKE $1 OR CAST(productid AS TEXT) ILIKE $1 OR CAST(warehouseid AS TEXT) ILIKE $1 ORDER BY id DESC LIMIT $2 OFFSET $3', [`%${value}%`, limit, offset]),
            pool.query('SELECT COUNT(*) FROM stocks WHERE CAST(quantity AS TEXT) ILIKE $1 OR CAST(productid AS TEXT) ILIKE $1 OR CAST(warehouseid AS TEXT) ILIKE $1', [`%${value}%`])
        ]);
        res.status(200).json(paginate(data.rows, parseInt(count.rows[0].count), page, limit));
    } catch (err) { res.status(500).json(err); }
};
...
const deleteById = async (req, res) => {
    try {
        const result = await pool.query('DELETE FROM stocks WHERE id=$1', [req.params.id]);
        if (result.rowCount === 0) return res.status(404).json({ message: 'stocks id does not match.' });
